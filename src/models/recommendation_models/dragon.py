...
class DRAGON(GeneralRecommender):
    def __init__(self, config, dataset):
        ...
        self.mm_audio_weight = config['mm_audio_weight']
        has_id = True
        ...
        if self.a_feat is not None:
            self.aduio_embedding = nn.Embedding.from_pretrained(self.a_feat, freeze=False)
            self.aduio_trs = nn.Linear(self.a_feat.shape[1], self.feat_embed_dim)
        ...
        if os.path.exists(mm_adj_file):
            ...
            if self.a_feat is not None:
                indices, audio_adj = self.get_knn_adj_mat(self.aduio_embedding.weight.detach())
                self.mm_adj = audio_adj
            if self.v_feat is not None and self.t_feat is not None and self.a_feat is not None:
                self.mm_adj = self.mm_image_weight * image_adj + self.mm_audio_weight * audio_adj + (1.0 - self.mm_image_weight - self.mm_audio_weight) * text_adj
                del text_adj
                del image_adj
                del audio_adj
            # TODO:
        ...
        self.interaction_matrix = self.sparse_mx_to_torch_sparse_tensor(train_interactions).float().to_dense()
        ...
        # pdb.set_trace()
        # Weixin: where 3 is the num of the modalities
        ...
        if self.a_feat is not None:
            self.a_drop_ze = torch.zeros(len(self.dropt_node_idx), self.a_feat.size(1)).to(self.device)
    ...
    def forward(self, interaction):
        ...
        if self.t_feat is not None:
            ...
            if representation is None:
                ...
                if self.construction == 'cat':
                    representation = torch.cat((representation, self.t_rep), dim=1)
                else:
                    representation += self.t_rep
        if self.a_feat is not None:
            self.a_rep, self.a_preference = self.a_gcn(self.edge_index_dropt, self.edge_index, self.a_feat)
            if representation is None:
                representation = self.a_rep
            else:
                if self.construction == 'cat':
                    representation = torch.cat((representation, self.a_rep), dim=1)
                else:
                    representation += self.a_rep
        ...
        if self.construction == 'weighted_sum':
            ...
            if self.a_rep is not None:
                self.a_rep = torch.unsqueeze(self.a_rep, 2)
                user_rep = self.a_rep[:self.num_user]
            if self.v_rep is not None and self.t_rep is not None and self.a_rep is not None:
                user_rep = torch.matmul(torch.cat((self.v_rep[:self.num_user], self.t_rep[:self.num_user], self.a_rep[:self.num_user]), dim=2), self.weight_u)
            # TODO
        ...
        if self.construction == 'weighted_max':
            ...
            user_rep = torch.cat((self.v_rep[:self.num_user], self.t_rep[:self.num_user], self.a_rep[:self.num_user]), dim=2)
    ...
    def calculate_loss(self, interaction):
        ...
        reg_embedding_loss_a = (self.a_preference[user] ** 2).mean() if self.a_preference is not None else 0.0

        reg_loss = self.reg_weight * (reg_embedding_loss_v + reg_embedding_loss_t + reg_embedding_loss_a)
        if self.construction == 'weighted_sum':
    ...
    def get_user_embedding(self, users):
        return self.result_embed[users]

    def get_user_aggregated_item_embedding(self, users):
        pretrained_item_representation = self.result_embed[self.n_users:]
        user_aggregated_item_embeddings = torch.mm(self.interaction_matrix[users], pretrained_item_representation)
        return user_aggregated_item_embeddings

    def save_pretrained_representation(self):
        pretrained_user_representation = self.result_embed[:self.n_users].cpu().detach().numpy()
        pretrained_item_representation = self.result_embed[self.n_users:].cpu().detach().numpy()
        with open(os.path.join(self.dataset_path, 'DARGON_user_representation.npy'), 'wb') as fu:
            np.save(fu, pretrained_user_representation, allow_pickle=True)
        with open(os.path.join(self.dataset_path, 'DARGON_item_representation.npy'), 'wb') as fi:
