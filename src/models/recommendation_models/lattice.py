...
class LATTICE(GeneralRecommender):
    def __init__(self, config, dataset):
        ...
        self.modal_weight = nn.Parameter(torch.Tensor([1.0, 1.0, 1.0]))
    ...
    def forward(self, adj, build_item_graph=False):
        ...
        if build_item_graph:
            ...
            if self.v_feat is not None:
                ...
                learned_adj = weight[0] * self.image_adj
                original_adj = weight[0] * self.image_original_adj
            if self.t_feat is not None:
                self.text_adj = build_sim(text_feats)
                self.text_adj = build_knn_neighbourhood(self.text_adj, topk=self.knn_k)
                learned_adj = weight[1] * self.text_adj if learned_adj is None else learned_adj + weight[1] * self.text_adj
                original_adj = weight[1] * self.text_original_adj if original_adj is None else original_adj + weight[1] * self.text_original_adj
            if self.a_feat is not None:
                self.audio_adj = build_sim(audio_feats)
                self.audio_adj = build_knn_neighbourhood(self.audio_adj, topk=self.knn_k)
                learned_adj = weight[2] * self.audio_adj if learned_adj is None else learned_adj + weight[2] * self.audio_adj
                original_adj = weight[2] * self.audio_original_adj if original_adj is None else original_adj + weight[2] * self.audio_original_adj
        ...
        if self.cf_model == 'ngcf':
            ego_embeddings = torch.cat((user_embeddings, self.item_id_embedding.weight), dim=0)
            ...
            return user_embeddings, self.item_id_embedding.weight + F.normalize(h, p=2, dim=1)
    ...
    def calculate_loss(self, interaction):
        ...
        self.pretrained_user_representation, self.pretrained_item_representation = self.forward(self.norm_adj, build_item_graph=self.build_item_graph)
    ...
    def save_pretrained_representation(self):
        pretrained_user_representation = self.pretrained_user_representation.cpu().detach().numpy()
        pretrained_item_representation = self.pretrained_item_representation.cpu().detach().numpy()
        with open(os.path.join(self.dataset_path, 'LATTICE_user_representation.npy'), 'wb') as fu:
            np.save(fu, pretrained_user_representation, allow_pickle=True)
        with open(os.path.join(self.dataset_path, 'LATTICE_item_representation.npy'), 'wb') as fi:
