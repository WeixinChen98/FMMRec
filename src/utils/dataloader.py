...
class TrainDataLoader(AbstractDataLoader):
    ...
    def __init__(self, config, dataset, batch_size=1, shuffle=False):
        ...
        # self.knn_k_uugraph = config['knn_k_uugraph']
        ...
        #     # if file exist?
        #     v_feat_file_path = os.path.join(dataset_path, config['biased_vision_feature_file'])
        #     t_feat_file_path = os.path.join(dataset_path, config['biased_text_feature_file'])
        ...
        #     image_uu_adj_file = os.path.join(dataset_path, 'biased_image_uu_adj_{}.pt'.format(self.knn_k_uugraph))
        #     if os.path.exists(image_uu_adj_file):
        #         image_uu_adj = torch.load(image_uu_adj_file)
        #     else:
        #         # load avail items for all uid
        #         user_indices = list(self.history_items_per_u.keys())
        #         image_user_embedding = torch.zeros(self.dataset.get_user_num(), self.image_embedding.weight.size(-1))
        #         # Iterate over each user's interaction history
        #         for u in user_indices:
        #             # Retrieve the indices of items the user has interacted with
        #             item_indices = torch.tensor(list(self.history_items_per_u[u]))
                    
        #             # Gather the corresponding item embeddings
        #             user_item_image_embeddings = self.image_embedding.weight[item_indices]
                    
        #             # Sum the item embeddings along the appropriate dimension to get the user embedding
        #             image_user_embedding[u] = torch.mean(user_item_image_embeddings, dim=0)
                
        #         image_uu_adj = build_sim(image_user_embedding.detach())
        #         image_uu_adj = build_knn_neighbourhood(image_uu_adj, topk=self.knn_k_uugraph)
        #         image_uu_adj = compute_normalized_laplacian(image_uu_adj)
        #         torch.save(image_uu_adj, image_uu_adj_file)
        #     self.image_uu_adj = image_uu_adj.cuda()

        # if self.t_feat is not None:
        #     self.text_embedding = nn.Embedding.from_pretrained(self.t_feat, freeze=False)
        #     text_uu_adj_file = os.path.join(dataset_path, 'biased_text_uu_adj_{}.pt'.format(self.knn_k_uugraph))
        #     if os.path.exists(text_uu_adj_file):
        #         text_uu_adj = torch.load(text_uu_adj_file)
        #     else:
        #         # load avail items for all uid
        #         user_indices = list(self.history_items_per_u.keys())
        #         text_user_embedding = torch.zeros(self.dataset.get_user_num(), self.text_embedding.weight.size(-1))
        #         # Iterate over each user's interaction history
        #         for u in user_indices:
        #             # Retrieve the indices of items the user has interacted with
        #             item_indices = torch.tensor(list(self.history_items_per_u[u]))
        ...
        #             # Gather the corresponding item embeddings
        #             user_item_text_embeddings = self.text_embedding.weight[item_indices]
                    
        #             # Sum the item embeddings along the appropriate dimension to get the user embedding
        #             text_user_embedding[u] = torch.mean(user_item_text_embeddings, dim=0)
                
        #         text_uu_adj = build_sim(text_user_embedding.detach())
        #         text_uu_adj = build_knn_neighbourhood(text_uu_adj, topk=self.knn_k_uugraph)
        #         text_uu_adj = compute_normalized_laplacian(text_uu_adj)
        #         torch.save(text_uu_adj, text_uu_adj_file)
        #     self.text_uu_adj = text_uu_adj.cuda()

        
        
    
    def get_mg_uu_adj(self):
        return self.image_uu_adj, self.text_uu_adj
    ...
    def _get_full_uids_sample(self):
        ...
        self.pr += self.step
        # TODO: add sensitive features of users
        # # TODO: extend to mulitple sensitive features
        # feat_tensor = torch.tensor([self.config['feature_columns'][0]].values).type(torch.LongTensor).to(self.device)
        # batch_tensor = torch.cat((batch_tensor, feat_tensor.unsqueeze(0)))
        return user_tensor
...
ALL_SUFFIX = '.inter'                 # all data file

# Recommender system related
USER = 'userID'                   # user column name
ITEM = 'itemID'                   # item column name
LABEL = 'x_label'                 # label column name
...
class DiscriminatorDataReader:
    def __init__(self, path, dataset_name, feature_columns, sep='\t', seq_sep=',', test_ratio=0.2):
        self.logger = getLogger()
        self.path = os.path.join(path, dataset_name)
        self.sep = sep
        self.seq_sep = seq_sep
        self.all_file = os.path.join(self.path, dataset_name + ALL_SUFFIX)
        self.train_attacker_file = os.path.join(self.path, dataset_name + '_' + '_'.join(feature_columns) + '.attacker' + TRAIN_SUFFIX)
        self.test_attacker_file = os.path.join(self.path, dataset_name + '_' + '_'.join(feature_columns) + '.attacker' + TEST_SUFFIX)
        self.all_df = pd.read_csv(self.all_file, sep='\t')

        # add feature info for discriminator and filters
        uid_iid_label = [USER, ITEM, LABEL]
        self.feature_columns = [name for name in self.all_df.columns.tolist() if name not in uid_iid_label and name in feature_columns]
    ...
    def _init_feature_df(self, all_df, test_ratio):
        self.logger.info('Initializing attacker train/test file...')
        feature_df = pd.DataFrame()
        all_df = all_df.sort_values(by=USER)
        all_group = all_df.groupby(USER)
        ...
        while sign:
            ...
            for f in self.feature_columns:
                ...
                if len(val_range) != len(test_range) and test_size != 0:
                    sign = True
                    break
                else:
                    sign = False
